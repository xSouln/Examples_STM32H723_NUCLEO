...
RECEIVED_PACKET rx_packet SN_RX_PACKET_MEM_SECTION;

// allocated at compile time - this is quite large for the stack
DEVICE_FIRMWARE_CHUNK dummy_chunk SN_DUMMY_CHUNK_MEM_SECTION;
...
BaseType_t sn_init(uint64_t *mac, uint16_t panid, uint8_t channel)
{
    ...
    // This has actual received packets in there
    xRxMailbox					= xQueueCreate(1, sizeof(RECEIVED_PACKET));

    // Flag to say if application accepted this packet.
    xRxMailbox_resp				= xQueueCreate(1, sizeof(SN_DATA_RECEIVED_RESPONSE));
    xGetNumPairsMailbox_resp	= xQueueCreate(1, sizeof(uint8_t));

    // queue with depth of one, and one parameter (bool)
    xSetPairingModeMailbox		= xQueueCreate(1, sizeof(PAIRING_REQUEST));
    ...
    // queue with depth of one, and one parameter (uint64_t)
    xIsDeviceOnlineMailbox		= xQueueCreate(1, sizeof(uint64_t));

    // queue with depth of one, and one parameter (uint64_t)
    xIsDeviceOnlineMailbox_resp	= xQueueCreate(1, sizeof(bool));

    // Queue to send mac address up to application to see if it has messages for that destination
    xGetNextMessageMailbox		= xQueueCreate(2, sizeof(uint64_t));

    // queue to send get_next_message info into SureNet
    xGetNextMessageMailbox_resp	= xQueueCreate(1, sizeof(MESSAGE_PARAMS));

    // queue for device awake message payload for app.
    xDeviceAwakeMessageMailbox	= xQueueCreate(1, sizeof(DEVICE_AWAKE_MAILBOX));

    // queue for pairing mode changed.
    xPairingModeHasChangedMailbox = xQueueCreate(1, sizeof(PAIRING_REQUEST));
	...
	// Send all device statuses to Register Map.
...
int SurnetStart_Task()
{
	surenet_task_handle =
			xTaskCreateStatic(sn_task, // Function that implements the task.
								...
								SURENET_TASK_PRIORITY, // Priority at which the task is created.
...
void sn_task(void *pvParameters)
{
    ...
    while(1)
    {
       ...
       // in SureNet Driver.
        ...
        if (xEventBits & SURENET_GET_PAIRMODE)
        {
        	// Received request for current pairing mode
            PAIRING_REQUEST resp;

            // this just retrieves a module level variable
            resp = sn_get_hub_pairing_mode();

            // put result in mailbox for Surenet-Interface
            xQueueSend(xGetPairmodeMailbox_resp, &resp, 10);
        }
        ...
        if(xEventBits & SURENET_GET_NUM_PAIRS)
        {
			// Received request for number of paired devices
            uint8_t num_pairs;
            num_pairs = sn_how_many_pairs();
            // put result in mailbox for Surenet-Interface
            xQueueSend(xGetNumPairsMailbox_resp, &num_pairs, 10);
        }
        ...
        if(xEventBits & SURENET_CLEAR_PAIRING_TABLE)
        {
        	// Received request for number of paired devices
        ...
        if(xEventBits & SURENET_TRIGGER_CHANNEL_HOP)
        {
        ...
        if (xEventBits & SURENET_GET_CHANNEL)
        {
        	// Received request to retrieve channel number
            u8result = snd_get_channel();
            // put result in mailbox for Surenet-Interface
			xQueueSend(xGetChannelMailbox_resp, &u8result, 10);
        }
        ...
        if(xEventBits & SURENET_GET_DEVICE_TABLE)
        {
            ...
            xQueueSend(xSendDeviceTableMailbox, &device_status, 10);
        }
        ...
        if(xEventBits & SURENET_GET_LAST_HEARD_FROM)
        {
        	// Main loop wants to know when we last heard from a device
			u32result = last_heard_from();
            xQueueSend(xGetLastHeardFromMailbox_resp, &u32result, 10);
        }
		...
		if (xQueueReceive(xDeviceFirmwareChunkMailbox, &device_firmware_chunk, 0) == pdPASS)
		{
        ...
        diff = get_microseconds_tick() - ageing_tick;

        // will trigger at 100,000
        if(diff > usTICK_MILLISECONDS * 100)
        {
            diff = diff / (usTICK_MILLISECONDS * 100);

            // cap it because the age used in the queues is only 8 bits.
            if(diff > 255)
            {
            	diff = 255;
            }
		...
		if (ping_stats.report_ping)
		{
			xQueueSend(xPingDeviceMailbox_resp, &ping_stats, 10);

			// just send the result the once.
			ping_stats.report_ping = false;
		}

		// run the state machine that oversees hub functions, e.g. send beacons, handle pairing requests etc. etc.
        can_i_sleep = hub_functions();

        // state machine to control detaching from all devices
        can_i_sleep |= detach_handler();

        // check the time we last chatted to a device and mark it offline if it was a while ago
        timeout_handler();

    	if(can_i_sleep == CAN_SLEEP)
		{
...
{
	// ensure that the SureNet idea of what pairing mode we are in is accurate
	// We need this because the pairing mode may change as part of Stack activity
	// (i.e. Association Successful).
    pairing_mode = new_state;

    // put result in mailbox for Surenet-Interface
    xQueueSend(xPairingModeHasChangedMailbox, &new_state, 0);
}
...
void sn_ping_device(uint64_t device, uint8_t value)
{
	...
	if (device_status[index].status.valid)
	{
		surenet_printf("Setting send_ping to true\r\n");

		// in case the mac was specified as 0, i.e. use the first one in the pairing table
		ping_stats.mac_address = device_status[index].mac_address;

		// This flag is used to indicate that a ping has been responded to and the ping_stats are updated
		ping_stats.found_ping = false;

		// flag to trigger conversation state machine to send a ping
		device_status_extra[index].send_ping = true;

		// value to be passed as part of the ping.
		device_status_extra[index].ping_value = value;
...
void sn_channel_control(uint8_t chn)
{
	...
	{
		if((chn >= START_CHANNEL) && (chn < (START_CHANNEL + NUMBER_OF_CHANNELS)))
		{
			snd_set_channel(chn);
		}
	}
...
{
    uint8_t index;
    if(convert_addr_to_index(mac_addr, &index))
    {
        if(device_status[index].status.online)
        {
            return true;
        }
    }
...
{
    uint8_t index;
	if(mac_addr == SURENET_UNASSIGNED_MAC)
	{
		return;
	}
...
BUSY_STATE detach_handler(void)
{
    ...
    switch(detach_state)
    {
        ...
        case DETACH_FIND_NEXT_PAIR:
            while(device_index < MAX_NUMBER_OF_DEVICES)
            {
                if((detach_device_bits) & (1ull << device_index))
                {
                    ...
                    // break out of this while() loop if this device needs clearing
                    break;
                }
                device_index++;
            }
            ...
            if(!device_status[device_index].status.valid)
            {
            	// if this entry is not valid, clear the entire entry in the pairing table.
            	//ensure connection table entry is fully reset
                memset((uint8_t*)&device_status[device_index], 0, sizeof(DEVICE_STATUS));
                //ensure connection table entry is fully reset
                memset((uint8_t*)&device_status_extra[device_index], 0, sizeof(DEVICE_STATUS_EXTRA));
                //update hubRegisterBank
        ...
        case DETACH_SEND_DETACH_COMMAND:
        	// handle retrying sending detach commands
			if(detach_retry_counter > MAX_DETACH_RETRIES)
            {
                ...
                //wont actually wait for ACK as we have already nulled send_detach
        ...
        case DETACH_WAIT_FOR_ACK:
        	// an ack will have arrived in the acknowledge_queue, so we need to check it
        	// send_detach[] is cleared by the main state machine if the necessary ACK is received
            if(!device_status_extra[device_index].send_detach)
            {
        ...
        case DETACH_FINISH:
			...
			{
				// go back to start and wait for the next trigger
...
{
	//override channel during pairing
	static uint8_t normal_channel = INITIAL_CHANNEL;
	...
	//call SureNetDriver to trigger the association process
	snd_pairing_mode(state);
...
BUSY_STATE hub_functions(void)
{
	...
	switch (hub_state)
	{
		...
		case HUB_STATE_CONVERSATION_START:
			...
			if (current_conversee >= MAX_NUMBER_OF_DEVICES)
			{
				...
				hub_state = HUB_STATE_HOP;
				for(i = 0; i < MAX_NUMBER_OF_DEVICES; i++)
				{
					if(device_status_extra[i].SecurityKeyUses >= MAX_SECURITY_KEY_USES)
					{
			...
			else
			{
				...
				|| (device_status_extra[current_conversee].sec_key_invalid >= MAX_BAD_KEY))
				{
					...
					for(i = 0; i < 16; i++)
					{
						SecurityKeyScramble[i] = 	SecurityKeys[current_conversee][i] ^ scramble[i];
					}

					// send the encryption key
					hub_conv_seq = sn_transmit_packet(PACKET_ENCRYPTION_KEY,
							device_status[current_conversee].mac_address,
							(uint8_t*)SecurityKeyScramble,
							SECURITY_KEY_SIZE, -1, false);
				...
				else
				{
					...
					#if PET_DOOR_DELAY
					if(device_status[current_conversee].status.device_type == DEVICE_TYPE_CAT_FLAP)
					{
		...
		case HUB_STATE_CONVERSATION_WAIT_FOR_SECURITY_ACK:
			...
			|| ((get_microseconds_tick() - hub_conv_timestamp) > TIMEOUT_SECURITY_ACK))
			{
				// didn't get the ack, so we will skip and move on to the next device.
				if(ack_reply == NACK_ARRIVED)
				{
		...
		case HUB_STATE_CONVERSATION_SEND:
			...
			if(device_status_extra[current_conversee].send_detach)
			{
				tx_retries = 0;
				hub_conv_timestamp = get_microseconds_tick();
				surenet_printf("Sending Detach\r\n");
				// send the detach command
				hub_conv_seq = sn_transmit_packet(PACKET_DETACH, device_status[current_conversee].mac_address, 0, 0, -1, false);
				// wait for DETACH_ACK.
				hub_state = HUB_STATE_CONVERSATION_WAIT_FOR_DETACH_ACK;
				break;
			}
			...
			DebugCounter.hub_state_conversation_send++;
			...
			if((result == pdPASS) && message_params.new_message)
			{
				// we have a new message to send
				hub_conv_timestamp = get_microseconds_tick();
				tx_retries = 0;

				DebugCounter.message_server_to_device++;

				hub_conv_seq = sn_transmit_packet(PACKET_DATA,
						device_status[current_conversee].mac_address,
						(uint8_t*)message_params.ptr,
						message_params.ptr->length, -1, false);

				surenet_log_add(SURENET_LOG_DATA_SEND, current_conversee);
				// wait for DATA_ACK.
				hub_state = HUB_STATE_CONVERSATION_WAIT_FOR_DATA_ACK;
			}
			else
			{
				...
				if(device_status_extra[current_conversee].send_ping)
				{
		...
		case HUB_STATE_SEND_PING:
			...
			ping_tx_result = sn_transmit_packet(PACKET_PING,
					device_status[current_conversee].mac_address,
					&device_status_extra[current_conversee].ping_value,
					1,-1, false);
			...
			if(ping_tx_result > -1)
			{
				...
				hub_state = HUB_STATE_SEND_PING_WAIT;
				hub_conv_timestamp = get_microseconds_tick();
			}
			else
			{
		...
		case HUB_STATE_END_OR_RECEIVE:
			...
			else if (device_status_extra[current_conversee].device_awake_status.data == DEVICE_HAS_DATA)
			{
				...
				sn_transmit_packet(PACKET_DEVICE_TX,
						device_status[current_conversee].mac_address,
						(uint8_t *)&dtx_payload,
						sizeof(DEVICE_TX_PAYLOAD), -1, false);
		...
		case HUB_STATE_CONVERSATION_WAIT_FOR_DETACH_ACK:
		...
		case HUB_STATE_CONVERSATION_WAIT_FOR_DATA_ACK:
			// wait for the ack to arrive, or timeout and resend
			...
			else if((get_microseconds_tick() - hub_conv_timestamp) > TIMEOUT_WAIT_FOR_DATA_ACK)
			{
				...
				&& ((get_microseconds_tick() - device_status[current_conversee].last_heard_from) < (usTICK_MILLISECONDS * 100)))
				{
					...
					sn_transmit_packet(PACKET_DATA,
							device_status[current_conversee].mac_address,
							(uint8_t*)message_params.ptr,
							message_params.ptr->length, hub_conv_seq, false);
		...
		case HUB_STATE_CONVERSATION_RECEIVE:
			// receive and acknowledge data from the device, until timeout or PACKET_END_SESSION
			if(conv_end_session_flag || ((get_microseconds_tick() - most_recent_rx_time) > usTICK_SECONDS))
			{
				// timeout if we've not received a new message in 1 sec.
				if(conv_end_session_flag)
				{
	...
	if(hub_state == HUB_STATE_CONVERSATION_START)
	{
		return CAN_SLEEP;
	}
...
void sn_send_chunk(uint8_t current_conversee,FIRMWARE_CHUNK *firmware_chunk)
{
	...
	// any left over from first segment
	seg2_length = ((firmware_chunk->len) > FIRST_SEGMENT_SIZE) ? firmware_chunk->len-FIRST_SEGMENT_SIZE : 0;
	...
	if (device_rcvd_segs[current_conversee] == 0x00)
	{
		...
		// length (trimmed to 72)
		payload[3] = seg1_length;

		// set status bit if this is the last segment
		payload[4] = (seg2_length == 0) ? DATA_SEGMENT_STATUS_LAST_CHUNK : 0;

		if (seg1_length > 0)
		{
			memcpy(&payload[PACKET_DATA_SEGMENT_HEADER_SIZE], firmware_chunk->data, seg1_length);
		}
		sn_transmit_packet(PACKET_DATA_SEGMENT,
				dest_mac,
				payload,
				seg1_length + PACKET_DATA_SEGMENT_HEADER_SIZE, -1, false);
	}
	...
	// Only send the 2nd segment if there's anything in it
	if (seg2_length > 0)
	{
		// second segment
		payload[0] = 1;

		// length (trimmed to 64)
		...
		if (seg2_length > 0)
		{
			memcpy(&payload[PACKET_DATA_SEGMENT_HEADER_SIZE],
					(firmware_chunk->data) + FIRST_SEGMENT_SIZE,
					seg2_length);
		}
		...
		sn_transmit_packet(PACKET_DATA_SEGMENT,
				dest_mac,
				payload,
				seg2_length + PACKET_DATA_SEGMENT_HEADER_SIZE, -1,false);
	}
	...
	// free this chunk so it won't get sent again.
	firmware_chunk->has_data = false;
}
...
ACK_STATUS has_data_ack_arrived(int16_t seq)
{
	...
	{
		if((data_acknowledge_queue[i].valid) && (data_acknowledge_queue[i].seq_acknowledged == seq))
		{
			data_acknowledge_queue[i].valid = 0;
			return !data_acknowledge_queue[i].ack_nack ? ACK_ARRIVED : NACK_ARRIVED;
		}
	}
...
ACK_STATUS has_ack_arrived(int16_t seq)
{
	...
	{
		if(acknowledge_queue[i].valid && (acknowledge_queue[i].seq_acknowledged == seq))
		{
			acknowledge_queue[i].valid = 0;

			return !acknowledge_queue[i].ack_nack ? ACK_ARRIVED : NACK_ARRIVED;
		}
	}
	return ACK_NOT_ARRIVED;
}
...
void age_data_acknowledge_queue(uint8_t ticks)
{
	...
	for(i = 0; i < DATA_ACKNOWLEDGE_QUEUE_SIZE; i++)
	{
		if (data_acknowledge_queue[i].valid)
		{
...
bool isMacAddressForUs(uint64_t destination)
{
    if(!destination || (destination == 0xffffffffffffffff))
	{
    	return true;
	}

    if(destination != *rf_mac)
	{
    	return false;
	}
...
SN_DATA_RECEIVED_RESPONSE sn_data_received(RECEIVED_PACKET *rx_packet)
{
    ...
    // Notify SureNet-Interface
    xQueueSendToFront(xRxMailbox, rx_packet, 0);
    ...
    // never got a result from SureNet-Interface
    return SN_TIMEOUT;
}
/**************************************************************
...
void convert_IEEE_to_sureflap(RECEIVED_PACKET *rx_packet, RX_BUFFER *rx_buffer)
{
    ...
    rx_packet->packet.header.packet_length = rx_buffer->ucBufferLength + sizeof(HEADER) - 2;

    // 2nd byte of payload is sequence number
    rx_packet->packet.header.sequence_number = rx_buffer->ucRxBuffer[1];

    // 1st byte of payload is packet type
    rx_packet->packet.header.packet_type = rx_buffer->ucRxBuffer[0];
    ...
    rx_packet->packet.header.crc = 0;

    // will get overwritten
    rx_packet->packet.header.rss = 0;
	...
	// copy payload
    memcpy(&rx_packet->packet.payload, &rx_buffer->ucRxBuffer[2], rx_buffer->ucBufferLength - 2);

    // calculate CRC on header & payload, probably not needed by SureNet
    calculated_crc = CRC16((uint8_t*)&rx_packet->buffer, rx_packet->packet.header.packet_length, 0xcccc);
...
bool sn_process_received_packet(RX_BUFFER *rx_buffer)
{
    ...
    // used to extract data from a PACKET_DEVICE_AWAKE
    PACKET_DEVICE_AWAKE_PAYLOAD *pda_payload;
    //we make a copy of the packet_device_awake data and send it to the application.
    ...
    if (!isMacAddressForUs(rx_packet.packet.header.destination_address))
    {
    	// Should never happen as Driver strips out packets not for us?
        surenet_printf("Packet not for us\r\n");
        // perfectly valid packet, just not for us!)
        return true;
    }
    else
    {
    	// packet is for us, so we need to process it. It could be a repeated packet.
    	// Note this only returns the index if the device is 'valid' as well as having a matching MAC address.
        device_index = convert_mac_to_index(rx_packet.packet.header.source_address);
        if((device_index < 0) || !device_status[device_index].status.associated)
        {
			if(rx_packet.packet.header.packet_type != PACKET_BLOCKING_TEST)
			{
				//not yet associated with this device (even if device is stored in NVM)
				return false;
			}
        ...
        if((device_index >= 0) && (device_index < MAX_NUMBER_OF_DEVICES))
        {
            ...
            {
            	//update 'online' flag NOT SURE IF IT IS ACTUALLY USED!!
                device_status[device_index].status.online = 1;
        ...
        // some repeated packets do have to be processed, (acknowledged)
        if (!is_packet_new(&rx_packet, &ack_payload[0]))
        {
            switch (rx_packet.packet.header.packet_type)
            {
				...
				case PACKET_DATA_ALT_ENCRYPTED:
                    ...
                    sn_transmit_packet(PACKET_DATA_ACK,
                    		rx_packet.packet.header.source_address,
							ack_payload,
							2, -1, false);
                    break;
        ...
        {
        	// this is not a repeated packet, so deal with it 'normally'
            surenet_printf("%08x: Processing packet of type %s\r\n",
            		get_microseconds_tick(),
					packet_names[rx_packet.packet.header.packet_type]);
            ...
            switch (rx_packet.packet.header.packet_type)
            {
                ...
                case PACKET_DATA:
                    ...
                    if (!rx_packet.packet.header.spare)
					{
                    	// decrypt payload
						sn_Decrypt(rx_packet.packet.payload,
									packet_length - sizeof(HEADER),
									device_index,
									rx_packet.packet.header.sequence_number);
					...
					if((rx_packet.packet.payload[2] - 1) > sizeof(RECEIVED_PACKET))
					{
						...
						{
                        	device_status_extra[device_index].sec_key_invalid++;
						}
                        return false;
                    }
					...
					pM = (uint8_t)GetParity((int8_t*)(&rx_packet.packet.payload[0]), rx_packet.packet.payload[2] - 1);
					...
					rx_packet.packet.header.packet_length--;
                    ...
                    if(are_we_paired_with_source(rx_packet.packet.header.source_address))
                    {
                        ...
                        sn_resp = sn_data_received(&rx_packet);
                        switch(sn_resp)
                        {
                            case SN_ACCEPTED:
                            	// message accepted, so send an ACK
                                ack_payload[1] = 0;
                                break;
                            ...
                            default:
                            	// message rejected, so send a NACK
                                ack_payload[1] = 0xff;
                                break;
                        }
                    }
                ...
                case PACKET_DATA_ACK:
                    ...
                    i = 0;
                    while((i < DATA_ACKNOWLEDGE_QUEUE_SIZE) && data_acknowledge_queue[i].valid)
                    {
                ...
                case PACKET_ACK:
                    ...
                    i = 0;
                    while ((i<ACKNOWLEDGE_QUEUE_SIZE) && acknowledge_queue[i].valid)
                    {
                    	// find the first free space in the queue
                        i++;
                    }
                ...
                case PACKET_DEVICE_STATUS:
                	//Allows any relevant initialisation details to be supplied by device
                    ...
                    else if(rx_packet.packet.payload[1] == DEVICE_SEND_KEY)
                    {
                ...
                case PACKET_DEVICE_AWAKE:
                    ...
                    if(are_we_paired_with_source(rx_packet.packet.header.source_address))
                    {
                        ...
                        if((device_index < MAX_NUMBER_OF_DEVICES) && (device_index >= 0))
                        {
                            ...
                            if(device_status[device_index].lock_status != pda_payload->lock_status
                            || !device_status[device_index].status.valid)
                            {
                            ...
                            if(device_status_extra[device_index].deviceMinutes != pda_payload->device_minutes)
                            {
                            	// minutes changed
                                ...
                                || (uptime_min_count == (device_index + 1)))
								{
									...
									// we do the data copying first to ensure we don't overwrite it
									rx_message->payload[4] = pda_payload->battery_voltage;
									// with any of the fixed values we write below.
									rx_message->payload[5] = pda_payload->device_hours;
									rx_message->payload[6] = pda_payload->device_minutes;
									rx_packet.packet.header.packet_type = PACKET_DATA;
									// rx_message->length (below) + sizeof(header) -1. Not sure why -1 is needed.
									rx_packet.packet.header.packet_length = 12 + 23;
									// COMMAND_SET_REG - This is not available as a #define in this part of the code
									rx_message->command = 2;
									// 2 for COMMAND_, 2 for length, 2 for reg address, 2 for num of reg, 3 for values, and 1 for parity
									rx_message->length = 12;
									// register number high byte
									rx_message->payload[0] = 0;
									// register number low byte
									rx_message->payload[1] = 33;
									// num of reg high byte
									rx_message->payload[2] = 0;
									// num of reg low byte
									rx_message->payload[3] = 3;
                    ...
                    else
                    {
                        ...
                        sn_transmit_packet(PACKET_REFUSE_AWAKE, rx_packet.packet.header.source_address, 0, 0, -1, false);
                    }
                    //end case PACKET_DEVICE_AWAKE:
                    break;
                ...
                case PACKET_PING_R:
                {
                	...
                	//cant use ~ping_seq directly, treated as uint32_t!!!
                    uint8_t tmp = ~ping_seq;
                    static uint8_t last_ping_val;
                    //force failure if sequence number doesn't match so increments nbad;
                    ping_stats.ping_value = last_ping_val + 1;

                    //Device inverts sequence number as a check
                    if(rx_packet.packet.payload[0] == tmp)
                    {
                    	//May want to use this for timing ping response time
                        ping_stats.reply_timestamp = get_microseconds_tick();
                        // stop repeated pings
                        ping_stats.found_ping = true;
                        // and report it's happened
						ping_stats.report_ping = true;
						ping_stats.num_good_pings++;
				...
				case PACKET_BLOCKING_TEST:
					blocking_sequence_no =  rx_packet.packet.payload[0]
											+ (rx_packet.packet.payload[1] << 8)
											+ (rx_packet.packet.payload[2] << 16)
											+ (rx_packet.packet.payload[3] << 24);

					// put result in mailbox for Surenet-Interface
            		xQueueSend(xBlockingTestMailbox, &blocking_sequence_no, 0);
					break;
...
void store_ack_response(uint64_t mac, uint8_t seq_no, uint8_t ack_response)
{
	...
	while(((received_sequence_numbers[i].src_mac != mac)
	|| (received_sequence_numbers[i].sequence_number != seq_no))
	&& (i<256))
	{
		i++;
	}
...
bool is_packet_new(RECEIVED_PACKET *rx_packet, uint8_t *ack_payload)
{
	...
	for(i = 0; i < RX_SEQ_BUFFER_SIZE; i++)
	{
		if (received_sequence_numbers[i].valid && (received_sequence_numbers[i].sequence_number == seq_num))
		{
			...
			if (received_sequence_numbers[i].src_mac == rx_packet->packet.header.source_address)
			{
				...
				// previous value sent
				ack_payload[1] = received_sequence_numbers[i].ack_response;
				// MAC addresses match, as does sequence number, so it's not a new packet
				return false;
			}
		}
	}
	...
	{
		if (!received_sequence_numbers[i].valid)
		{
			received_sequence_numbers[i].valid = lifetime;
 ...
 * Returns         :
 **************************************************************/
// Deleted old comment to stop Tom chirruping.
void timeout_handler(void)
{
    static int i=0;
	BaseType_t retval;
	...
	&& device_status[i].status.online)
	{
		surenet_printf("Device %d has gone offline\r\n",i);
		// set device as offline (but leave it still valid)
		device_status[i].status.online = 0;
		retval = surenet_update_device_table_line(&device_status[i], i, false, false);
	}

	if(retval == pdPASS)
	{
		if(i++ >= MAX_NUMBER_OF_DEVICES)
		{
			i = 0;
		}
    }
...
void sn_CalculateSecretKey(uint8_t device_index)
{
	...
	uint8_t ChaCha_SureKey_Constant[CHACHA_MAX_KEY_SZ];

	const uint8_t 	obfuscated_constant[] =
	{
...
{
	//uint32_t i;
	if(pair_index > MAX_NUMBER_OF_DEVICES)
	{
		return;
	}
	...
	switch(device_status_extra[pair_index].encryption_type)
	{
		case SURENET_CRYPT_BLOCK_XTEA:
			// decrypt payload
			XTEA_64_Decrypt(data,length, SecurityKeys[pair_index]);
			//for( i = 0; i < length; i++ ){ crypt_printf(" %02x", data[i]); }
			break;
...
void dump_surenet_log(void)
{
	...
	for(i = 0; i < NUM_SURENET_LOG_ENTRIES; i++)
	{
		...
		zprintf(CRITICAL_IMPORTANCE,"[%02d] %09d %9d %s %03d\r\n",
				i,
				surenet_log_entry[i].timestamp,
				delta,
				log_names[surenet_log_entry[i].activity],
				surenet_log_entry[i].parameter);
	}
}
//------------------------------------------------------------------------------
uint64_t get_microseconds_tick_64(void)
{
	...
	if((last_timestamp & 0x80000000) && !(timestamp & 0x80000000))
	{
		// just rolled over
...
void longprint(uint64_t value)
{
	...
	do
	{
		...
		if(this_digit > 0)
		{
			...
			// start printing digits now as we've hit a non zero digit
			started = true;
		}
		else
		{
