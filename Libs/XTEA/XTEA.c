...
XTEA_64_BlockT XTEA_64_Encode(XTEA_64_BlockT source, XTEA_64_KeyT key)
{
	...
	{
		source.HalfWords[0] += (((source.HalfWords[1] << 4) ^ (source.HalfWords[1] >> 5)) + source.HalfWords[1]) ^ (sum + key[sum & 3]);
		sum += delta;
		source.HalfWords[1] += (((source.HalfWords[0] << 4) ^ (source.HalfWords[0] >> 5)) + source.HalfWords[0]) ^ (sum + key[(sum >> 11) & 3]);
	}
...
void XTEA_64_CTR_Encode(uint8_t* source, uint16_t length, XTEA_64_KeyT key)
{
	...
	XTEA_64_BlockT	nonce	= {0};
	XTEA_64_BlockT	crypted	= {0};
...
{
	uint32_t		i;
	XTEA_64_BlockT	MIC = XTEA_64_GenerateMIC(source, length, (uint16_t*)key);
...
{
	XTEA_64_BlockT	received_MIC, calculated_MIC;
